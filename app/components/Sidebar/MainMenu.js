...
function MainMenu(props) {
  ...
  const checkRights = (routeName) => {
    if ((userAttr.rights && userAttr.rights.indexOf(routeName) > -1) || routeName === 'home') return true;
    return false;
  };

  const getMenus = menuArray => menuArray.map((item, index) => {
    if (checkRights(item.key)) {
  ...
  return (
    <div>
      <div style={{ display: 'none' }}>{userAttr.name}</div>
      {getMenus(dataMenu)}
    </div>
  );
}
